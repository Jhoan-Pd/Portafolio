...
import type { Project as ProjectItem } from '@/types/portfolio';

export default function Projects() {
  const section = usePortfolioSection('projects');
  const { language } = useLanguage();
  const items = section?.items as ProjectItem[] | undefined;
  const projects = useMemo(() => items ?? [], [items]);
  const defaultDescription = section?.defaultDescription ?? '';
  ...
  const title = section?.title?.toUpperCase() ?? (language === 'es' ? 'PROYECTOS' : 'PROJECTS');
